...
class NotValidExpressionException(Exception):
    """
...
def replace_word_tokens(string, language):
    ...
    if set(string) & set_operators == set():
        if 'sqrt' not in string and 'log' not in string:
            raise PostfixTokenEvaluationException('The postfix expression does not have operators')
